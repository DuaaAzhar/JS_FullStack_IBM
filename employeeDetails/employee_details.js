const employees = [
      ...
      { id: 3, name: 'Alice Smith', age: 28, department: 'HR', salary: 45000 , specialization: '.Net'},
      { id: 4, name: 'Bob Johnson', age: 35, department: 'Finance', salary: 60000 , specialization: 'PHP'},
...
function displayEmployees(){
    const totalEmployees= employees.map( 
        employee => `${employee.id}: ${employee.name} - ${employee.age} - ${employee.department} - $${employee.salary} </p>`
    ).join('');
...
function findEmployeeBySpecialization(specialization) {
    ...
    if(foundEmployee){
        ...
        `<p>${foundEmployee.id}: ${foundEmployee.name} - ${foundEmployee.age} - ${foundEmployee.department} - ${foundEmployee.salary} - ${foundEmployee.specialization}</p>`;
    }
    else{
